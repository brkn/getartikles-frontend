...
export function ArticleList({sortBy, filterDate}: ArticleListProps) {
  const [
    sortedArticles,
    setSortedArticles
  ] = useState(articles);
    ...
    function filterByDate() {
      ...
      switch (filterDate) {
        ...
        case "year":
          ...
          break;
        case "all":
          oldestAllowedDate.setFullYear(1900);
          break;
      }
    ...
    function sortArticles() {
      ...
      function sortByNew() {
        setSortedArticles(
          [...sortedArticles].sort((articleA, articleB) => {
            return (
              Number(articleB.publish_date) - Number(articleA.publish_date)
            );
          })
        );
      }
      ...
      function sortByOld() {
        setSortedArticles(
          [...sortedArticles].sort((articleA, articleB) => {
            return (
              Number(articleA.publish_date) - Number(articleB.publish_date)
            );
          })
        );
      }
